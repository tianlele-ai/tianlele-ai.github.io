...
(function ($) {
  ...
  function indexInit() {
    ...
    $(document).on('mouseenter mouseleave', '.works-active .active .work', function (e) {
      var $this = $(this),
        $thisOriginalHeight = $this.outerHeight(true);
      ...
      if (winWidth > 799) {
        if (e.type === 'mouseenter') {
          ...
          TweenLite.to($this, .4, {
            height: (winWidth / workColls - 80) + 'px',
            top: "0",
            ease: Quint.easeInOut
          });
          TweenLite.to($('.work-outer.active .mask-black'), .5, {
  ...
  function setColumns(workColls) {
    var itemWidth = Math.floor(winWidth / ((workColls < 3) ? workColls : 3));
  ...
  function contactInit() {
    var subpageTitleHeight = $('.l-title').height(),
      ...
      contactDataHeight = $('.contact-data').height() - 145;
